...
\`\`\`graphql query search{ projectByName\(
...
\) { id, branches, productionEnvironment, pullrequests, gitUrl, openshift { id }, groups{ id name groups { id name } members { role user { id email } } } } }
